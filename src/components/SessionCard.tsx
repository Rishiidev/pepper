...
const CAPTURE_LABELS: Record<string, { label: string; color: string }> = {
  ...
  auto_window_close: { label: 'Auto-captured', color: 'text-violet-700 dark:text-violet-400 bg-violet-500/10 border-violet-500/20' },
  auto_idle: { label: 'Idle Capture', color: 'text-blue-700 dark:text-blue-400 bg-blue-500/10 border-blue-500/20' },
  keyboard_shortcut: { label: 'Shortcut', color: 'text-cyan-700 dark:text-cyan-400 bg-cyan-500/10 border-cyan-500/20' },
...
export const SessionCard: React.FC<SessionCardProps> = ({ session }) => {
  ...
  return (
    <div className="relative bg-surface-card border border-border/70 rounded-2xl p-5 hover:border-pepper-500/40 hover:bg-surface-hover/80 shadow-md transition-all duration-300 space-y-4 group">
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-start gap-3.5 min-w-0">
          ...
          <div className="min-w-0 space-y-0.5">
            <div className="flex items-center gap-2">
              <span className="text-xs uppercase font-bold tracking-widest text-pepper-400">
                {session.projectName || 'General'}
              </span>
              {session.isPinned && <Pin className="w-3 h-3 text-pepper-400 fill-pepper-400" />}
              {session.isFavorite && <Star className="w-3 h-3 text-amber-700 dark:text-amber-400 fill-amber-400" />}
              <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 ml-1" title="Active Context" />
            </div>
        ...
        <div className="flex items-center gap-2 shrink-0">
          {session.captureType && session.captureType !== 'manual' && (
            <span className={`text-xs font-bold px-1.5 py-0.5 rounded-md border flex items-center gap-1 ${CAPTURE_LABELS[session.captureType]?.color || CAPTURE_LABELS.manual.color}`}>
      ...
      <div className="bg-surface/40 border border-border/40 rounded-xl p-3.5 space-y-2">
        {session.sessionIntent && (
          <div className="flex items-center gap-1.5">
            <Brain className="w-3.5 h-3.5 text-violet-700 dark:text-violet-400 shrink-0" />
            <span className="text-xs text-violet-700 dark:text-violet-400 font-semibold">{session.sessionIntent}</span>
          </div>
        )}
      ...
      {session.domainClusters && session.domainClusters.length > 0 && (
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {session.domainClusters.slice(0, 5).map((domain) => (
            <span key={domain} className="text-xs font-bold font-mono px-2 py-0.5 rounded-md bg-surface border border-border/60 text-text-secondary">
              {domain}
            </span>
          ))}
        </div>
      ...
      >
        <span className="text-xs font-bold uppercase tracking-wider text-text-muted">Previews:</span>
        <div className="flex items-center -space-x-1.5 overflow-hidden cursor-pointer">
          ...
          {session.tabCount > 5 && (
            <span className="inline-flex items-center justify-center w-5.5 h-5.5 rounded-full bg-surface border border-border text-xs font-bold text-text-secondary">
              +{session.tabCount - 5}
            </span>
          )}
        </div>
      ...
      <div className="flex flex-wrap items-center gap-y-2 gap-x-4 text-xs text-text-muted border-t border-border/40 pt-3">
        ...
        {session.estimatedRamSavedMb && (
          <>
            <span>&bull;</span>
            <span className="text-emerald-700 dark:text-emerald-400 font-semibold">{session.estimatedRamSavedMb} MB Saved</span>
          </>
        )}
        {focusSeconds >= 60 && (
          <>
      ...
      <div className="flex items-center justify-between gap-3 pt-1">
        <button
          ...
          className="flex-1 flex items-center justify-center gap-2 py-2 px-4 rounded-xl bg-pepper-500 hover:bg-pepper-600 text-white font-bold text-xs transition-colors shadow-lg active:scale-[0.98]"
        >
        ...
        <div className="flex items-center gap-1">
          <button
            ...
            className={`px-2.5 py-2 rounded-xl border text-xs font-bold transition-colors ${
          ...
          <button aria-label={`${session.isFavorite ? 'Unfavorite' : 'Favorite'} ${session.name}`} aria-pressed={!!session.isFavorite}
            ...
            className={`p-2 rounded-xl border border-border/50 hover:bg-surface transition-colors ${
              session.isFavorite ? 'text-amber-700 dark:text-amber-400 bg-amber-500/5' : 'text-text-muted hover:text-text-primary'
            }`}
            title="Favorite"
          >
              ...
              >
                ...
                <div className="flex items-center gap-2 shrink-0">
                  {duration !== undefined && duration > 0 && (
                    <span className="text-xs font-mono text-text-muted bg-surface-card border border-border/40 px-1.5 py-0.5 rounded">
