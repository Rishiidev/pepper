      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <div className="bg-surface-card border border-border/80 rounded-2xl p-4 space-y-1">
          <div className="flex items-center justify-between text-text-muted">
            <span className="text-xs font-semibold uppercase tracking-wider">Memory Saved</span>
            <Cpu className="w-4 h-4 text-emerald-700 dark:text-emerald-400" />
          </div>
          <div className="text-xl font-bold text-emerald-700 dark:text-emerald-400 font-mono">{ramGb} GB</div>
          <div className="text-xs text-text-muted">RAM Memory Recovered</div>
        </div>

        <div className="bg-surface-card border border-border/80 rounded-2xl p-4 space-y-1">
          <div className="flex items-center justify-between text-text-muted">
            <span className="text-xs font-semibold uppercase tracking-wider">Tabs Saved</span>
            <Layers className="w-4 h-4 text-pepper-400" />
          </div>
          <div className="text-xl font-bold text-text-primary font-mono">{totalTabs}</div>
          <div className="text-xs text-text-muted">Across all browser windows</div>
        </div>
        ...
        <div className="bg-surface-card border border-border/80 rounded-2xl p-4 space-y-1">
          ...
          <div className="text-xs text-text-muted">Active projects &amp; sessions</div>
        </div>

        <div className="bg-surface-card border border-border/80 rounded-2xl p-4 space-y-1">
          <div className="flex items-center justify-between text-text-muted">
            <span className="text-xs font-semibold uppercase tracking-wider">Time Recovered</span>
            <Clock className="w-4 h-4 text-amber-700 dark:text-amber-400" />
          </div>
          <div className="text-xl font-bold text-amber-700 dark:text-amber-400 font-mono">{hoursRecovered} Hours</div>
          <div className="text-xs text-text-muted">Context switching avoided</div>
        </div>
      </div>
      ...
      {(autoCaptures > 0 || totalSessions > 0) && (
        <div className="flex items-center gap-4 px-4 py-3 bg-surface-card border border-border/60 rounded-2xl text-xs">
          <div className="flex items-center gap-2">
            <Brain className="w-4 h-4 text-violet-700 dark:text-violet-400" />
            <span className="text-text-muted font-semibold">Memory Engine:</span>
          </div>
          <div className="flex items-center gap-4 text-text-secondary">
            <span className="flex items-center gap-1.5">
              <Zap className="w-3.5 h-3.5 text-violet-700 dark:text-violet-400" />
