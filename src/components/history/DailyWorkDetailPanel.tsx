    ...
    <div className="bg-surface-card border border-border/80 rounded-3xl p-6 space-y-6 shadow-2xl animate-slide-up select-none">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 text-center">
        <div className="p-3 rounded-2xl bg-surface border border-border/60 space-y-1">
          <span className="text-xs text-text-muted font-bold uppercase tracking-wider block">Focused Time</span>
          <span className="font-extrabold font-mono text-sm text-pepper-400">{formatSecs(record.focusedSeconds)}</span>
        </div>
        <div className="p-3 rounded-2xl bg-surface border border-border/60 space-y-1">
          <span className="text-xs text-text-muted font-bold uppercase tracking-wider block">Active Flow</span>
          <span className="font-extrabold font-mono text-sm text-emerald-700 dark:text-emerald-400">{formatSecs(record.activeWorkSeconds)}</span>
        </div>
        <div className="p-3 rounded-2xl bg-surface border border-border/60 space-y-1">
          <span className="text-xs text-text-muted font-bold uppercase tracking-wider block">Breaks Logged</span>
          <span className="font-extrabold font-mono text-sm text-blue-700 dark:text-blue-400">{formatSecs(record.breakSeconds)}</span>
        </div>
        <div className="p-3 rounded-2xl bg-surface border border-border/60 space-y-1">
          <span className="text-xs text-text-muted font-bold uppercase tracking-wider block">Focus Sessions</span>
          <span className="font-extrabold text-sm text-text-primary">{record.sessionsCount}</span>
        </div>
        <div className="p-3 rounded-2xl bg-surface border border-border/60 space-y-1">
          <span className="text-xs text-text-muted font-bold uppercase tracking-wider block">Workspaces</span>
      ...
      <div className="p-4 rounded-2xl bg-surface border border-border/80 space-y-2">
        <div className="flex items-center justify-between">
          ...
          <span className="text-xs font-mono text-emerald-700 dark:text-emerald-400 font-bold px-2 py-0.5 rounded bg-emerald-500/10 border border-emerald-500/20">
                ...
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2.5">
                      ...
                      <span className="text-xs font-bold uppercase px-2 py-0.5 rounded bg-surface-card border border-border text-text-secondary">
                        {sess.mode}
                      </span>
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        ...
                        className="flex items-center gap-1 text-xs font-bold text-pepper-400 hover:underline px-2.5 py-1 rounded bg-pepper-500/10 border border-pepper-500/20"
                      >
                  ...
                  {sess.aiSummary && (
                    <p className="text-xs text-text-muted leading-relaxed font-medium italic pl-1 border-l border-pepper-500/30">
                      "{sess.aiSummary}"
                    </p>
                  )}
                </div>
      ...
      <div className="space-y-2 pt-2 border-t border-border/60">
        ...
        {isEditingNotes ? (
          ...
          <div className="text-xs text-text-muted italic">No manual notes added for this date.</div>
        )}
      </div>
    </div>
  );
